...
class CatalogActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val peliculasCatalog: RecyclerView = findViewById(R.id.movies_catalog)
        val seriesCatalog: RecyclerView = findViewById(R.id.series_catalog)

        // LayoutManager con 3 columnas
        val peliculasLayoutManager = GridLayoutManager(this, 3)
        val seriesLayoutManager = GridLayoutManager(this, 3)


        // Configurar el Adapter con la lista de películas y series
        adapter = PeliculaAdapter(peliculas)
        seriesAdapter = PeliculaAdapter(series)

        peliculasCatalog.layoutManager = peliculasLayoutManager
        seriesCatalog.layoutManager = seriesLayoutManager

        peliculasCatalog.adapter = adapter
        seriesCatalog.adapter = seriesAdapter

        // Configurar el padding para evitar que el contenido quede cubierto por las barras del sistema
    ...
    class PeliculaAdapter(private val peliculas: ArrayList<Pelicula>) : RecyclerView.Adapter<PeliculaAdapter.PeliculaViewHolder>() {
        ...
        inner class PeliculaViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
            ...
            fun bind(pelicula: Pelicula) {
                imageView.setImageResource(pelicula.image)
                titleTextView.text = pelicula.titulo
